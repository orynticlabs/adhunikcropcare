...
export async function createOryCMSAdminUser(input: UserInput, actor: CurrentOryCMSAdmin) {
  ...
  const [row] = await orycmsPrisma.$queryRaw<AdminUserRow[]>`
    INSERT INTO orycms_users (id, email, "passwordHash", status, "roleId", "fullName", "mobileNumber", "profilePhoto", "emailVerified", "createdAt", "updatedAt")
    VALUES (gen_random_uuid(), ${data.email}, ${passwordHash}, ${data.status}, ${roleId}::uuid, ${data.fullName}, ${data.mobileNumber}, ${data.profilePhoto}, ${data.emailVerified}, now(), now())
    RETURNING id, email, status, "createdAt", "updatedAt", "deletedAt", "fullName", "mobileNumber", "profilePhoto", "emailVerified", "lastLoginAt", ${data.role} AS "roleName"
  `
  return toDTO(row)
}
...
export async function updateOryCMSAdminUser(id: string, input: UserInput, actor: CurrentOryCMSAdmin) {
  ...
  const [row] = await orycmsPrisma.$queryRaw<AdminUserRow[]>`
    ...
    RETURNING id, email, status, "createdAt", "updatedAt", "deletedAt", "fullName", "mobileNumber", "profilePhoto", "emailVerified", "lastLoginAt", ${role} AS "roleName"
  `
