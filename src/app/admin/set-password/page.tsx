...
import { CheckCircle2, AlertCircle, Lock, ShieldCheck, Loader2, ArrowRight } from "lucide-react"
...
export default function SetPasswordPage({
  ...
  useEffect(() => {
    ...
    setLoading(true)
    fetch(`/api/orycms/auth/set-password?token=${encodeURIComponent(token)}`)
      .then(async (res) => {
        ...
        if (res.ok && json.success && json.data) {
          ...
          setValid(false)
          setErrorMessage(json.error?.message || "Invitation token is invalid or expired.")
        }
      })
      .catch((err) => {
        setValid(false)
        setErrorMessage(err instanceof Error ? err.message : "Failed to validate invitation token.")
      })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setErrorMessage("")
    try {
      const res = await fetch("/api/orycms/auth/set-password", {
        method: "POST",
      ...
      if (!res.ok || !json.success) {
        throw new Error(json.error?.message || "Failed to set password.")
      }
      setSuccess(true)
    } catch (err) {
        ...
        <div className="bg-white py-8 px-6 shadow-xl border border-border sm:rounded-2xl sm:px-10">
          {loading ? (
            <div className="py-12 text-center space-y-3">
              ...
              <p className="text-sm font-medium text-foreground">Validating invitation token…</p>
            </div>
          ) : success ? (
            <div className="py-6 text-center space-y-4">
              ...
              <h3 className="text-xl font-bold text-foreground">Password Setup Complete!</h3>
              <p className="text-sm text-gray-600 leading-relaxed">
                Your email <strong className="text-gray-900">{userEmail}</strong> has been verified and your account is now active.
              </p>
              <div className="pt-4">
                <Link
                  href="/admin"
            ...
            <div className="py-6 text-center space-y-4">
              ...
              <h3 className="text-lg font-bold text-foreground">Invitation Link Invalid</h3>
              <p className="text-sm text-red-600 bg-red-50 p-3 rounded-lg border border-red-200">
                {errorMessage}
              </p>
              <p className="text-xs text-gray-500">
                Invitation links are valid for 24 hours and single-use only. Please request a new invitation link from your Super Admin.
              </p>
              <div className="pt-2">
                <Link
                  href="/admin"
            ...
            <form className="space-y-5" onSubmit={handleSubmit}>
              ...
              <div>
                ...
                <div className="mt-1 relative rounded-md shadow-xs">
                  ...
                  <input
                    ...
                    className="block w-full pl-9 pr-3 py-2.5 text-sm rounded-xl border border-border bg-white focus:outline-none focus:ring-2 focus:ring-[#FF5A20]"
                  />
                </div>
              </div>
              ...
              <div className="pt-2">
                <button
                  type="submit"
                  disabled={submitting}
                  className="w-full flex items-center justify-center gap-2 rounded-xl bg-foreground py-3 px-4 text-sm font-semibold text-white shadow-xs hover:!bg-[#FF5A20] hover:!text-white transition-colors disabled:opacity-60 cursor-pointer select-none"
                >
                  {submitting ? (
                    <>
