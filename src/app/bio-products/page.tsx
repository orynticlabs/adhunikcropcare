...
export default function BioProductsPage() {
  return (
    <div className="min-h-screen overflow-hidden bg-[#eff5ef] text-[#17382d]">
      ...
      <main>
        <section className="relative isolate min-h-[760px] overflow-hidden bg-[#062f24] pb-24 pt-36 text-white sm:pt-44">
          ...
          <div className="relative mx-auto grid max-w-7xl gap-12 px-4 lg:grid-cols-[1fr_.9fr] lg:items-center">
            <div>
              ...
              <div className="mt-9 flex gap-3"><a href="#cultures" className="inline-flex h-12 items-center gap-2 rounded-full bg-[#e9c46a] px-7 text-sm font-bold text-[#17382d] hover:text-[#17382d]">Enter the lab <ArrowRight className="h-4 w-4" /></a><Link href="/contact" className="inline-flex h-12 items-center rounded-full border border-white/20 px-7 text-sm font-bold text-white hover:text-white">Get guidance</Link></div>
            </div>
        ...
        <section id="cultures" className="py-20 sm:py-28">
          <div className="mx-auto max-w-7xl px-4">
            <div className="text-center"><p className="text-xs font-bold uppercase tracking-[.22em] text-[#689c30]">Meet the cultures</p><h2 className="mt-4 font-display text-5xl sm:text-7xl">A community below ground.</h2></div>
            <div className="mt-14 grid gap-px overflow-hidden rounded-[2.75rem] bg-[#bfcdbf] sm:grid-cols-2">
              {CULTURES.map(([Icon,title,copy],i)=>{const I=Icon as typeof Leaf;return <div key={title as string} className="relative bg-white p-8 sm:p-10"><span className="absolute right-7 top-5 font-display text-6xl text-[#689c30]/10">0{i+1}</span><I className="h-8 w-8 text-[#689c30]" /><h3 className="mt-12 font-display text-4xl">{title as string}</h3><p className="mt-4 max-w-md leading-7 text-[#647269]">{copy as string}</p></div>})}
            </div>
          </div>
        </section>
        ...
        <section className="bg-[#dce9cb] py-20 sm:py-28">
          <div className="mx-auto grid max-w-7xl gap-12 px-4 lg:grid-cols-[.9fr_1.1fr] lg:items-center">
            ...
            <div><Sparkles className="h-8 w-8 text-[#689c30]" /><h2 className="mt-7 font-display text-5xl leading-tight sm:text-7xl">Biology needs the right conditions.</h2><p className="mt-6 max-w-xl text-lg leading-8 text-[#5e6f63]">Microbial inputs are living systems. Moisture, temperature, storage, timing, compatibility, and application practice influence their field performance.</p>
        ...
        <section className="py-20 sm:py-28">
          <div className="mx-auto max-w-7xl px-4"><div className="rounded-[3rem] bg-[#062f24] p-8 text-white sm:p-12 lg:flex lg:items-center lg:justify-between lg:p-16"><div><p className="text-xs font-bold uppercase tracking-[.2em] text-[#e9c46a]">Explore the bio range</p><h2 className="mt-4 max-w-3xl font-display text-5xl">Bring beneficial biology into your crop program.</h2></div><Link href="/products?q=Bio%20Products" className="mt-8 inline-flex h-12 items-center gap-2 rounded-full bg-[#bdd879] px-7 font-bold text-[#17382d] hover:text-[#17382d] lg:mt-0">View bio products <ArrowRight className="h-4 w-4" /></Link></div></div>
        </section>
      </main>
      <SiteFooter />
    </div>
